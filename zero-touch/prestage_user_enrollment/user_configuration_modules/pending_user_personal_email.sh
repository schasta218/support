...
#       Module Name: pending_user_personal_email
#       Module Version: 1.0
#
#       Details: This module can be used to takeover existing **PENDING**
#       users during the DEP enrollment process. The input fields "Personal Email"
#       is used to query the "Description" attribute for existing JumpCloud users.
#
#       Only **PENDING** users can be activated using this workflow. If you wish
#       to takeover existing active JumpCloud users use a user configuration
#       module which includes a secret. The "Description" field for users must
...
PersonalEmail=$(defaults read $DEP_N_USER_INPUT_PLIST "Enter Your Personal Email Address")
...
while [ "$LOCATED_ACCOUNT" == "False" ]; do
    ...
    userSearch=$(
        curl -s \
            ...
            -H "x-api-key: ${APIKEY}" \
            -d '{"filter":[{"activated":false,"description":"'${PersonalEmail}'"}],"fields":["username"]}' \
